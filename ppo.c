...
#define NUM_EPOCHS 10      // Number of optimization epochs per batch
...
int collect_rollout(Sim* sim, Net* policy, double** act, double** states, double** actions, double* rewards, double* old_log_probs) {
    ...
    while(steps < MAX_STEPS && !is_terminated(sim->quad)) {
        ...
        if(t_control <= t_physics) {
            ...
            for(int i = 0; i < 4; i++) {
                ...
                // Precomputed log(2π) ≈ 1.8378770664093453
                double z = (actions[steps][i] - mean) / std;
                old_log_probs[steps * 4 + i] = -0.5 * (1.8378770664093453 + 
                    2.0 * log(std) + z * z);
            }
...
void update_policy(Net* policy, double** states, double** actions, double* returns, double* old_log_probs, int steps, double** act, double** grad) {
    for(int epoch = 0; epoch < NUM_EPOCHS; epoch++) {
        for(int t = 0; t < steps; t++) {
            ...
            for(int i = 0; i < 4; i++) {
                ...
                // 3. Compute current log probability of the action
                // log(p(x)) = -0.5 * (log(2π) + 2log(σ) + z²)
                double log_prob = -0.5 * (1.8378770664093453 + 2.0 * log(std) + z * z);
                
                // 4. Compute PPO probability ratio
                // r(θ) = π_θ(a|s) / π_θ_old(a|s) = exp(log(π_θ(a|s)) - log(π_θ_old(a|s)))
                double ratio = exp(log_prob - old_log_probs[t * 4 + i]);
                
                // 5. Compute clipped ratio for PPO objective
                // clip(r(θ), 1-ε, 1+ε)
                double clipped_ratio = fmax(fmin(ratio, 1.0 + PPO_EPSILON), 
                                         1.0 - PPO_EPSILON);
                
                // 6. Compute entropy of the Gaussian distribution
                // H = 0.5 * (log(2πe) + 2log(σ))
                double entropy = 0.5 * (2.837877066 + 2.0 * log(std));
                
                // 7. Use returns as advantage (A = G - V, but we skip V)
                // Center returns to reduce variance
                double advantage = returns[t] - returns[t] * 0.5;
                
                // 8. PPO objective: min(r(θ)A, clip(r(θ), 1-ε, 1+ε)A)
                double unclipped_obj = ratio * advantage;
                double clipped_obj = clipped_ratio * advantage;
                bool use_clipped = (clipped_obj < unclipped_obj);
                
                // 9. Compute gradients based on selected objective
                double selected_ratio = use_clipped ? clipped_ratio : ratio;
                
                // 10. Gradient for mean (through both ratio and bounds)
                double dmean = (selected_ratio * advantage) * (z / std);
                grad[4][i] = (dmean * dsquash(act[4][i], mean_min, mean_max) + 
                            ALPHA * entropy);
                
                // 11. Gradient for std (more complex due to multiple dependencies)
                // Direct effect on log_prob: ∂log_prob/∂σ = (z² - 1)/σ
                // Effect through mean bounds: ∂mean/∂σ = -4.0 * dsquash
                ...
                grad[4][i + 4] = (selected_ratio * advantage * dstd + 
                                 ALPHA * (1.0 / std)) * 
...
int main(int argc, char** argv) {
    ...
    // Allocate network activation and gradient buffers
    ...
    double** old_log_probs = malloc(NUM_ROLLOUTS * sizeof(double*));  // New for PPO
    ...
    for(int r = 0; r < NUM_ROLLOUTS; r++) {
        ...
        old_log_probs[r] = malloc(MAX_STEPS * 4 * sizeof(double));  // 4 actions per step
    ...
    for(int iter = 1; iter <= NUM_ITERATIONS; iter++) {
        ...
        for(int r = 0; r < NUM_ROLLOUTS; r++) {
            steps[r] = collect_rollout(sim, policy, act, states[r], actions[r], 
                                     rewards[r], old_log_probs[r]);
            double ret = rewards[r][0];  // Total return for this rollout
        ...
        // PPO updates: multiple epochs over the same data
        for(int r = 0; r < NUM_ROLLOUTS; r++) {
            update_policy(policy, states[r], actions[r], rewards[r], 
                         old_log_probs[r], steps[r], act, grad);
        }
    ...
    for(int r = 0; r < NUM_ROLLOUTS; r++) {
        ...
        free(actions[r]);
        free(rewards[r]);
        free(old_log_probs[r]);  // Free PPO buffers
    }
    ...
    free(actions);
    free(rewards);
    free(old_log_probs);  // Free PPO buffer pointer
    free(steps);
    free(act);
    free(grad);
    free_net(policy);
