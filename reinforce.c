...
int main(int argc, char** argv) {
    ...
    for(int iter = 0; iter < iterations; iter++) {
        ...
        // Arrays to store data from all rollouts
        ...
        // Allocate memory for all rollouts
        ...
        for(int r = 0; r < NUM_ROLLOUTS; r++) {
            ...
            total_steps += rollout_steps[r];
        }

        // Update policy using all collected rollouts
        for(int r = 0; r < NUM_ROLLOUTS; r++) {
            update_policy(net, all_states[r], all_actions[r], 
                         all_rewards[r], rollout_steps[r], act, grad);
        }

        // Free memory for all rollouts
        for(int r = 0; r < NUM_ROLLOUTS; r++) {
            for(int i = 0; i < MAX_STEPS; i++) {
                free(all_states[r][i]);
