...
int main(int argc, char** argv) {
    ...
    for(int iter = 0; iter < iterations; iter++) {
        ...
        #pragma omp parallel
        {
            ...
            double** local_act = malloc(5 * sizeof(double*));
            for(int i = 0; i < 5; i++) {
                local_act[i] = malloc(net->sz[i] * sizeof(double));
            }

            #pragma omp for reduction(+:sum_returns) schedule(dynamic)
            for(int r = 0; r < NUM_ROLLOUTS; r++) {
                rollout_steps[r] = collect_rollout(sims[thread_id], net, local_act, 
                                                 all_states[r], 
                                                 all_actions[r], 
                                                 all_rewards[r]);
        ...
        // Sort indices (sequential)
        qsort_r(rollout_indices, NUM_ROLLOUTS, sizeof(int), 
                compare_returns_with_context, rollout_returns);

        // Update policy using top 20% of rollouts (sequential)
        int num_top_rollouts = NUM_ROLLOUTS / 5;
